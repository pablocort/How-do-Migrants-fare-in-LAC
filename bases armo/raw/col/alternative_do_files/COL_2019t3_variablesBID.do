 ...
 * Se tiene acceso al servidor tÃƒÂ©cnicamente al interior del BID.
...
label define region_BID_c 1 "CentroamÃƒÂ©rica_(CID)" 2 "Caribe_(CCB)" 3 "Andinos_(CAN)" 4 "Cono_Sur_(CSC)"
...
la var factor_ch "Factor de expansiÃƒÂ³n del hogar"
...
g zona_c = clase == 1
la var zona_c "Zona del paÃƒÂ­s"
...
************
g str3 pais_c = "COL"
la var pais_c "PaÃƒÂ­s"

**********
***anio***
**********
g anio_c = 2019
la var anio_c "AÃƒÂ±o de la encuesta"
...
la var factor_ci "Factor de expansiÃƒÂ³n del individuo"
...
*****************
	...
	la var relacion_ci "RelaciÃƒÂ³n con el jefe del hogar"
	la de relacion_ci 	1 "Jefe/a" 				///
						2 "Esposo/a" 			///
						3 "Hijo/a" 				///
						4 "Otros parientes" 	///
						5 "Otros no parientes" 	///
						6 "Empleado/a domÃƒÂ©stico/a"
...
**********
	g edad_ci = p6040
	la var edad_ci "Edad del individuo (aÃƒÂ±os)"
...
******************
	...
	la var nconyuges_ch "NÃƒÂºmero de cÃƒÂ³nyuges"

***************
***nhijos_ch***
***************
	bys idh_ch: egen nhijos_ch = sum(relacion_ci == 3)
	la var nhijos_ch "NÃƒÂºmero de hijos"

******************
***notropari_ch***
******************
	bys idh_ch: egen notropari_ch = sum(relacion_ci == 4)
	la var notropari_ch "NÃƒÂºmero de otros familiares"

********************
***notronopari_ch***
********************
	bys idh_ch: egen notronopari_ch = sum(relacion_ci == 5)
	la var notronopari_ch "NÃƒÂºmero de no familiares"

****************
***nempdom_ch***
****************
	bys idh_ch: egen nempdom_ch = sum(relacion_ci == 6)
	la var nempdom_ch "NÃƒÂºmero de empleados domÃƒÂ©sticos"
...
by idh_ch, sort: egen byte nmiembros_ch=sum(relacion_ci>0 & relacion_ci<=5)
	la var nmiembros_ch "NÃƒÂºmero de familiares en el hogar"

*****************
***nmayor21_ch***
*****************
by idh_ch, sort: egen byte nmayor21_ch=sum((relacion_ci>0 & relacion_ci<=5) & (edad_ci>=21 & edad_ci<=98))
	la var nmayor21_ch "NÃƒÂºmero de familiares mayores a 21 aÃƒÂ±os"

*****************
***nmenor21_ch***
*****************
by idh_ch, sort: egen byte nmenor21_ch=sum((relacion_ci>0 & relacion_ci<=5) & (edad_ci<21))
	la var nmenor21_ch "NÃƒÂºmero de familiares menores a 21 aÃƒÂ±os"

*****************
***nmayor65_ch***
*****************
by idh_ch, sort: egen byte nmayor65_ch=sum((relacion_ci>0 & relacion_ci<=5) & (edad_ci>=65 & edad_ci!=.))
	la var nmayor65_ch "NÃƒÂºmero de familiares mayores a 65 aÃƒÂ±os"

****************
***nmenor6_ch***
****************
by idh_ch, sort: egen byte nmenor6_ch=sum((relacion_ci>0 & relacion_ci<=5) & (edad_ci<6))
	la var nmenor6_ch "NÃƒÂºmero de familiares menores a 6 aÃƒÂ±os"

****************
***nmenor1_ch***
****************
by idh_ch, sort: egen byte nmenor1_ch=sum((relacion_ci>0 & relacion_ci<=5) & (edad_ci<1))
	la var nmenor1_ch "NÃƒÂºmero de familiares menores a 1 aÃƒÂ±o"
...
label var condocup_ci "Condicion de ocupaciÃƒÂ³n de acuerdo a def de cada pais"
...
label var instcot_ci "instituciÃƒÂ³n a la cual cotiza"
label define instcot_ci 1 "Fondo privado" 2 "ISS, Cajanal" 3 "RegÃƒÂ­menes especiales (FFMM, Ecopetrol etc)" 4 "Fondo Subsidiado (Prosperar,etc.)" 
...
/************************************************************************************************************
* 3. CreaciÃƒÂ³n de nuevas variables de SS and LMK a incorporar en Armonizadas
************************************************************************************************************/
...
*MGD30/11/2015: segun el documento metodologico se genera condicionada a si recibe ingreso por pension.  VarÃƒÂ­a ligeramente el resultado.
* La serie hacia atrÃƒÂ¡s se genera condicionando al ingreso no con la variable binaria.
...
label var categoinac_ci "CondiciÃƒÂ³n de inactividad"
...
******************
	...
	la var categopri_ci "CategorÃƒÂ­a ocupacional"
...
******************
	...
	la var categosec_ci "CategorÃƒÂ­a ocupacional trabajo secundario"
...
*****************
	...
	*replace nempleos_ci = 2 if emp_ci == 1 & p7040 == 1
	la var nempleos_ci "NÃƒÂºmero de empleos" 
/*
*****************
***firmapeq_ci***
*****************
...
*****************
	...
	la var spublico_ci "Personas que trabajan en el sector pÃƒÂºblico"
...
**************
	...
	la var ocupa_ci "OcupaciÃƒÂ³n laboral"
	la de ocupa_ci 	1 "Profesional y tÃƒÂ©cnico"																///
					...
					6 "Trabajador agrÃƒÂ­cola y afines"														///
					7 "Obrero no agrÃƒÂ­cola, conductores de mÃƒÂ¡quinas y vehÃƒÂ­culos de transporte y similares" 	///
					8 "Fuerzas armadas" 																	///
					9 "Otras ocupaciones"
...
*************
	...
	la def rama_ci 	1 "Agricultura, caza, silvicultura y pesca"				///
					2 "ExplotaciÃƒÂ³n de minas y canteras"						///
...
****************
	...
	la var durades_ci "DuraciÃƒÂ³n del desempleo en meses"
...
*Independientes actividad principal
	recode p6760 (0=0.5) // JLC: Â¿Por quÃ©? -> YL: pienso que es para justificar que gana porque trabajo al menos 15 dias. si se deja 0 es como ganar sin trabajar y no se sabria cuantos dias exactamente, es proxy.
...
*******************
	...
	la var nrylmpri_ch "Hogares con algÃƒÂºn miembro que no respondiÃƒÂ³ por ingresos"
...
************
	...
	la var luz_ch  "La principal fuente de iluminaciÃƒÂ³n es electricidad"
...
*************
	...
	la var des1_ch "Tipo de desague inadecuado (unimproved) segÃƒÂºn MDG"
	la def des1_ch 	0 "No tiene servicio sanitario" 				///
					1 "Conectado a red general o cÃƒÂ¡mara sÃƒÂ©ptica" 	///
					2 "Letrina o conectado a pozo ciego" 			///
					3 "Desemboca en rÃƒÂ­o o calle"
...
*************
	...
	la var des2_ch "Tipo de desague sin incluir definiciÃƒÂ³n MDG"
	la def des2_ch 	0 "No tiene servicio sanitario" 								///
					1 "Conectado a red general, cÃƒÂ¡mara psÃƒÂ©ptica, pozo o letrina" 	///
...
*************
	...
	la var piso_ch "Materiales de construcciÃƒÂ³n del piso"  
...
**************
	...
	la var pared_ch "Materiales de construcciÃƒÂ³n de las paredes"
...
**************
	g techo_ch = .
	la var techo_ch "Materiales de construcciÃƒÂ³n del techo"
...
**************
	...
	la var resid_ch "MÃƒÂ©todo de eliminaciÃƒÂ³n de residuos"
	la de resid_ch 	0 "RecolecciÃƒÂ³n pÃƒÂºblica o privada" 	///
...
**************
	...
	la var telef_ch "El hogar tiene servicio telefÃƒÂ³nico fijo"
...
*************
	...
	la var auto_ch "El hogar posee automÃ³vil particular"
...
*****************
	...
	la var internet_ch "El hogar posee conexiÃ³n a Internet"
...
************
	...
	la var cel_ch "El hogar tiene servicio telefÃ³nico celular"
...
****************
	g vivitit_ch = .
	la var vivitit_ch "El hogar posee un tÃ­tulo de propiedad"
...
saveold "$ruta\harmonized\\COL\\GEIH\data_arm\COL_2019t3_BID.dta", replace
