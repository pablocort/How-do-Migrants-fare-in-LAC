...
gen lp_ci= 3486.6 if zona_c==1
replace lp_ci= 1762.6 if zona_c==0
...
gen lpe_ci = 1743.3 if zona_c==1
replace lpe_ci= 1320.3 if zona_c==0
...
*****************
gen aguafuente_ch =.
replace aguafuente_ch = 1 if dv105==1 & dv106<=2
...
replace aguafuente_ch = 10 if dv105==9 | dv105==2 | dv105==3 |dv105==8 | (dv105==. & jefe_ci!=.)
...
* PTMC: Programa del Bono Vida Mejor (antes Programa del Bono 10Mil) (dh209 dh211 dh212) 
* PNC: 	Sin PNC
...
gen ing_ptmc  = 0
replace ing_ptmc=dh211 
replace ing_ptmc = 0 if dh211==. |  dh211==99999

* Bono vida mejor
gen percibe_ptmc_ci  = (dh209==1)
bys idh_ch: egen ptmc_ch  = max(percibe_ptmc_ci)

replace ptmc_ch= 1 if (ing_ptmc>0 & ing_ptmc!=.)

**No es posible construir las variables asociadas a PNC
gen ing_pension  = 0
gen pnc_ci = 0
