"""
build_jeremy_caribbean_unweighted.py  --  Unweighted version of jeremy_caribbean
==================================================================================
Output: out/jeremy/2026-05-13/jeremy_caribbean_unweighted.xlsx

Identical structure to jeremy_caribbean.xlsx but ALL computations use raw
observation counts (N) instead of survey weights (factor_ci).

Labor sheets  : unweighted rates (simple mean over non-missing obs)
education_abs : raw N per education group (unweighted count of persons)
education_pct : unweighted % share (cat N / total N)

Use this to inspect actual sample sizes behind the weighted estimates.
"""
...
# ---------------------------------------------------------------------------
# Unweighted mean helper  (replaces _wavg — ignores factor_ci entirely)
# ---------------------------------------------------------------------------
def _umean(series):
...
# ---------------------------------------------------------------------------
# 1. Build trend data (unweighted)
# ---------------------------------------------------------------------------
print("Computing unweighted trend indicators ...")
trend = {}
...
for ind_key, (col, _) in INDICATORS.items():
    ...
    for country in CARIBBEAN:
        ...
        for period in sorted(dfc["periodo_c"].unique()):
            ...
            for grp in GROUPS:
                s  = dfp.loc[dfp[GROUP_COL] == grp, col]
                mu = _umean(s)
                n  = _n(s)
...
# ---------------------------------------------------------------------------
# 2. Build education data — unweighted counts and shares
# ---------------------------------------------------------------------------
EDU_3CAT_MAP = {
...
print("Computing unweighted education distribution ...")
edu_abs = {}
edu_pct = {}
...
for country, period in RECENT_WAVES.items():
    ...
    for cat in EDU_ORDER[:-1]:
        ...
        for grp in GROUPS:
            ...
            total = len(dfg)
            cat_n = len(dfg[dfg["edu_3cat"] == cat])
            abs_row[grp] = cat_n
            pct_row[grp] = round(cat_n / total * 100, 2) if total > 0 else np.nan
...
# ---------------------------------------------------------------------------
# 3. Write Excel — same visual style as weighted version
# ---------------------------------------------------------------------------
C_NAVY   = "1D3557"
C_TEAL   = "1A7A6E"
C_ORANGE = "C55A11"
...
def write_snapshot_table(ws, start_row, ind_key, hdr_color):
    title_text = "BLZ / BRB / SUR / GUY  —  Single-wave snapshot (unweighted)"
    ws.cell(start_row, 1, title_text).font      = Font(bold=True, size=11, color="FFFFFF")
    ws.cell(start_row, 1).fill                  = _fill(C_NAVY)
    ws.cell(start_row, 1).alignment             = Alignment(horizontal="left", vertical="center")
    ws.row_dimensions[start_row].height         = 20
    ...
    hr = start_row + 1
    for ci, lbl in enumerate(["Country", "Native %", "Native N", "Migrant %", "Migrant N"], start=1):
    ...
    for ri, country in enumerate(SNAP_COUNTRIES):
        ...
        nat_pct = last.get("Native",   np.nan)
        nat_n   = last.get("Native_N", np.nan)
        mig_pct = last.get("Migrant",  np.nan)
        mig_n   = last.get("Migrant_N",np.nan)
        ...
        for ci, (val, fmt) in enumerate(
            [(nat_pct, "0.00"), (nat_n, "#,##0"), (mig_pct, "0.00"), (mig_n, "#,##0")],
            start=2
        ):
            dc = ws.cell(dr, ci, val if pd.notna(val) else None)
            dc.font          = Font(size=9)
...
for ind_key, (_, label) in INDICATORS.items():
    ...
    # BLZ / BRB / SUR / GUY: combined single-wave snapshot (% + N columns)
    current_row = write_snapshot_table(ws, current_row, ind_key, hdr_color)

    # DOM: full 7-wave trend — include N columns
    dom_tbl = trend[ind_key]["DOM"]
    current_row = write_country_table(
        ws, current_row, "DOM", f"all waves ({len(dom_tbl)})",
        dom_tbl, "Period / Year", hdr_color, COUNTRY_FILLS["DOM"],
        num_format="0.00",
    )
...
# Education sheets (unweighted counts and shares)
for sheet_label, data_dict, num_fmt, sheet_title in [
    ("education_abs", edu_abs, "#,##0",
     "Education level — unweighted observation count (N)"),
    ("education_pct", edu_pct, "0.00",
     "Education level — unweighted % share within group"),
]:
...
print("\nUnweighted education N (most recent wave):")
for country in CARIBBEAN:
    print(f"\n  {COUNTRY_LABEL[country]} ({RECENT_WAVES[country]})")
    print(edu_abs[country].to_string())
