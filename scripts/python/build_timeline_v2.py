"""
HDMF Timeline v2
Grupo A (existentes): COL(8) PER(7) ECU(8) CHL(3) = 26 rondas  |  6-10 Abril 2026
Grupo B (nuevos):     DOM(8) ARG(7) PRY(8) URY(7) CRI(8) PAN(7) = 45 rondas  |  13-22 Abril 2026
Gran total: 71 rondas en 10 paises
"""
import os
import openpyxl
from openpyxl.styles import PatternFill, Font, Alignment, Border, Side

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'HDMF_Timeline_v2.xlsx')
...
DAY_COLS_A = ['Lun 6', 'Mar 7', 'Mie 8', 'Jue 9', 'Vie 10']
DAY_COLS_B = ['Lun 13', 'Mar 14', 'Mie 15', 'Jue 16', 'Vie 17', 'Lun 20', 'Mar 21', 'Mie 22']

# ═══════════════════════════════════════════════════════════════════════════════
# GRUPO A — paises existentes
# ═══════════════════════════════════════════════════════════════════════════════
TASKS_A = [
...
(1,  'Auditoria de datos y scope',                  'TODOS','--',    'Setup',   {1}, 'Listar rondas faltantes por pais'),
...
(26, 'CHL 2022a   dict.check + clonar + run',      'CHL','2022a',  'Clonar',{4}, 'Clonar desde CHL_2024. CASEN bienal: ronda previa a 2024'),
...
# ═══════════════════════════════════════════════════════════════════════════════
# GRUPO B — paises nuevos   |   13-22 Abril 2026  (8 dias habiles)
# DOM=8 | ARG=7 | PRY=8 | URY=7 | CRI=8 | PAN=7 = 45 rondas
# Todos nuevos -> survey_mapper + stata_generator primero, luego clones
# Dias: 1=Lun13 2=Mar14 3=Mie15 4=Jue16 5=Vie17 6=Lun20 7=Mar21 8=Mie22
# ═══════════════════════════════════════════════════════════════════════════════
TASKS_B = [
('SEC','DIA 1  |  Lun 13 Abr  |  Setup + Descargas paralelas + Metadata', None,None,'SEC',set(),''),
...
('SEC','DIA 2  |  Mar 14 Abr  |  DOM: nuevo script + rondas 2018-2024', None,None,'SEC',set(),''),
(9,  'DOM 2025t3  survey_mapper + stata_generator (NUEVO)',  'DOM','2025t3','Nuevo',  {2}, 'Pegar agents/survey_mapper.md -> map_labor/demographics/migration/income. Generar variablesBID.do para DOM'),
...
(11, 'DOM 2023t3 a 2018t3  (6 clones)',                     'DOM','23-18', 'Clonar', {2}, '6 clones encadenados. Verificar var migracion en cada ronda (ENCFT reemplaza ENFT desde 2017)'),

('SEC','DIA 3  |  Mie 15 Abr  |  ARG: nuevo script + rondas 2018-2023 + Indicadores DOM+ARG', None,None,'SEC',set(),''),
(12, 'ARG 2024t3  survey_mapper + stata_generator (NUEVO)', 'ARG','2024t3','Nuevo',  {3}, 'CH15 = lugar nacimiento (1-5). ARG excluye zonas rurales -- documentar en metadata'),
(13, 'ARG 2023t3  dict.check + clonar + run',               'ARG','2023t3','Clonar', {3}, 'Clonar desde 2024t3. EPH tiene cambio metodologico acumulativo desde 2016'),
(14, 'ARG 2022t3 a 2018t3  (5 clones)',                     'ARG','22-18', 'Clonar', {3}, '5 clones. 2020t2: modo telefono (ASPO). Documentar con *CHANGED'),
(15, 'Indicadores DOM+ARG  (Python parcial)',               'CROSS','--',  'Indic.', {3}, 'Ejecutar hdmf_2.py con DOM+ARG. Verificar pais_c y tendencias empleo/migracion'),

('SEC','DIA 4  |  Jue 16 Abr  |  PRY: nuevo script + rondas 2018-2024', None,None,'SEC',set(),''),
(16, 'PRY 2025t3  survey_mapper + stata_generator (NUEVO)', 'PRY','2025t3','Nuevo',  {4}, 'EPHC continua desde 2017. Verificar lugar_nacimiento en DDI ANDA antes de mapear'),
(17, 'PRY 2024t3  dict.check + clonar + run',               'PRY','2024t3','Clonar', {4}, 'Clonar desde 2025t3'),
(18, 'PRY 2023t3 a 2018t3  (6 clones)',                     'PRY','23-18', 'Clonar', {4}, '6 clones. Sin gaps documentados. Verificar que migrante_ci disponible desde 2018'),

('SEC','DIA 5  |  Vie 17 Abr  |  URY: nuevo script + rondas 2018-2023 + Indicadores PRY+URY', None,None,'SEC',set(),''),
(19, 'URY 2024a   survey_mapper + stata_generator (NUEVO)', 'URY','2024a', 'Nuevo',  {5}, 'ECH anual. Tiene var migrante (no-migrante / uruguayo-retornado / extranjero) y pais nacimiento. N migrantes pequeno por ano -- considerar pooling'),
(20, 'URY 2023a  dict.check + clonar + run',                'URY','2023a', 'Clonar', {5}, 'Clonar desde 2024a'),
(21, 'URY 2022a a 2018a  (5 clones)',                       'URY','22-18', 'Clonar', {5}, '5 clones. Sin gaps. ECH excluye viviendas colectivas -- puede subestimar migrantes recientes'),
(22, 'Indicadores PRY+URY  (Python parcial)',               'CROSS','--',  'Indic.', {5}, 'Ejecutar hdmf_2.py con PRY+URY. Verificar education, labor, formalizacion'),

('SEC','DIA 6  |  Lun 20 Abr  |  CRI: nuevo script + rondas 2018-2024', None,None,'SEC',set(),''),
(23, 'CRI 2025a   survey_mapper + stata_generator (NUEVO)', 'CRI','2025a', 'Nuevo',  {6}, 'ENAHO anual (julio). Tiene condicion migratoria y lugar nacimiento. Fuente oficial extranjeros en CRI'),
(24, 'CRI 2024a  dict.check + clonar + run',                'CRI','2024a', 'Clonar', {6}, 'Clonar desde 2025a'),
(25, 'CRI 2023a a 2018a  (6 clones)',                       'CRI','23-18', 'Clonar', {6}, '6 clones. Sin gaps. 2020-2021: protocolos adaptados pero datos completos'),

('SEC','DIA 7  |  Mar 21 Abr  |  PAN: nuevo script + rondas 2018-2023 + Indicadores CRI+PAN', None,None,'SEC',set(),''),
(26, 'PAN 2024a   survey_mapper + stata_generator (NUEVO)', 'PAN','2024a', 'Nuevo',  {7}, 'EML anual. ADVERTENCIA: var migracion no confirmada en DDI publico. Verificar en ILO surveyLib antes de mapear. Si ausente: migrante_ci=. para todas las rondas'),
(27, 'PAN 2023a  dict.check + clonar + run',                'PAN','2023a', 'Clonar', {7}, 'Clonar desde 2024a. 2020: modo telefono (EMLT) -- documentar con *CHANGED'),
(28, 'PAN 2022a a 2018a  (5 clones)',                       'PAN','22-18', 'Clonar', {7}, '5 clones. Mes de referencia varia por ano (agosto/septiembre/octubre/abril). Documentar en metadata'),
(29, 'Indicadores CRI+PAN  (Python parcial)',               'CROSS','--',  'Indic.', {7}, 'Ejecutar hdmf_2.py con CRI+PAN. Revisar population_origin, labor, formalizacion'),

('SEC','DIA 8  |  Mie 22 Abr  |  Validacion + Indicadores finales + QC', None,None,'SEC',set(),''),
(30, 'Validacion global  (45 BID.dta nuevos)',              'TODOS','--',  'Validar',{8}, 'Agente validator: rangos, missings<30%, binary (0/1/.), factor_ci>0. Documentar anomalias'),
(31, 'Indicadores finales  hdmf_2.py completo (6 paises)', 'TODOS','--',  'Indic.', {8}, 'Ejecutar hdmf_2.py. Verificar pais_c.value_counts() incluye DOM/ARG/PRY/URY/CRI/PAN'),
(32, 'Indicadores laborales: empleo, desempleo, PEA',      'TODOS','--',  'Indic.', {8}, 'Revisar labor_status_pivot. Saltos 2020 esperados'),
(33, 'Indicadores formalizacion',                           'TODOS','--',  'Indic.', {8}, 'formality_pivot. Verificar DOM y PAN si formal_ci disponible'),
(34, 'Indicadores educativos',                              'TODOS','--',  'Indic.', {8}, 'edu_distribution_pivot + edu_years. Comparar migrantes vs nativos'),
(35, 'Indicadores poblacion y origen migrante',             'TODOS','--',  'Indic.', {8}, 'top_mig_countries por pais. CRI: nicaraguenses. URY: argentinos/brasileros. PAN: verificar'),
(36, 'QC + codebooks + documentacion final',                'TODOS','--',  'QC',     {8}, 'Revisar figuras. Generar harmonization_codebook por pais. Git commit todo'),
]
...
INV_B = [
    ...
    ('DOM',2025,'2025t3','Solicitar BCRD','Nuevo script',2,'Ronda mas reciente. Crear script base con survey_mapper'),
    ('ARG',2018,'2018t3','Disponible INDEC','Clonar+Run',3,'Clonar 2019t3. ARG excluye zonas rurales'),
    ('ARG',2019,'2019t3','Disponible INDEC','Clonar+Run',3,'Clonar 2020t3'),
    ('ARG',2020,'2020t3','Disponible INDEC','Clonar+Run',3,'2020t2: modo telefono (ASPO). Documentar *CHANGED'),
    ('ARG',2021,'2021t3','Disponible INDEC','Clonar+Run',3,'Clonar 2022t3'),
    ('ARG',2022,'2022t3','Disponible INDEC','Clonar+Run',3,'Clonar 2023t3'),
    ('ARG',2023,'2023t3','Disponible INDEC','Clonar+Run',3,'Clonar 2024t3'),
    ('ARG',2024,'2024t3','Disponible INDEC','Nuevo script',3,'Ronda mas reciente. CH15 = lugar nacimiento (1-5)'),
    ('PRY',2018,'2018t3','Disponible INE', 'Clonar+Run', 4,'Clonar 2019t3. Verificar migrante_ci en DDI ANDA'),
    ('PRY',2019,'2019t3','Disponible INE', 'Clonar+Run', 4,'Clonar 2020t3'),
    ('PRY',2020,'2020t3','Disponible INE', 'Clonar+Run', 4,'Sin gap documentado. Verificar ajustes COVID'),
    ('PRY',2021,'2021t3','Disponible INE', 'Clonar+Run', 4,'Clonar 2022t3'),
    ('PRY',2022,'2022t3','Disponible INE', 'Clonar+Run', 4,'Clonar 2023t3'),
    ('PRY',2023,'2023t3','Disponible INE', 'Clonar+Run', 4,'Clonar 2024t3'),
    ('PRY',2024,'2024t3','Disponible INE', 'Clonar+Run', 4,'Clonar 2025t3'),
    ('PRY',2025,'2025t3','Disponible INE', 'Nuevo script',4,'Ronda mas reciente. Crear script base'),
    ('URY',2018,'2018a', 'Disponible INE', 'Clonar+Run', 5,'Clonar 2019a. N migrantes pequeno por ano'),
    ('URY',2019,'2019a', 'Disponible INE', 'Clonar+Run', 5,'Clonar 2020a'),
    ('URY',2020,'2020a', 'Disponible INE', 'Clonar+Run', 5,'Sin gap. ECH anual continua'),
    ('URY',2021,'2021a', 'Disponible INE', 'Clonar+Run', 5,'Clonar 2022a'),
    ('URY',2022,'2022a', 'Disponible INE', 'Clonar+Run', 5,'Clonar 2023a'),
    ('URY',2023,'2023a', 'Disponible INE', 'Clonar+Run', 5,'Clonar 2024a'),
    ('URY',2024,'2024a', 'Disponible INE', 'Nuevo script',5,'Ronda mas reciente. Tiene migrante+pais_nacimiento'),
    ('CRI',2018,'2018a', 'Disponible INEC','Clonar+Run', 6,'Clonar 2019a'),
    ('CRI',2019,'2019a', 'Disponible INEC','Clonar+Run', 6,'Clonar 2020a'),
    ('CRI',2020,'2020a', 'Disponible INEC','Clonar+Run', 6,'Protocolos adaptados COVID. Datos completos'),
    ('CRI',2021,'2021a', 'Disponible INEC','Clonar+Run', 6,'Clonar 2022a'),
    ('CRI',2022,'2022a', 'Disponible INEC','Clonar+Run', 6,'Clonar 2023a'),
    ('CRI',2023,'2023a', 'Disponible INEC','Clonar+Run', 6,'Clonar 2024a'),
    ('CRI',2024,'2024a', 'Disponible INEC','Clonar+Run', 6,'Clonar 2025a'),
    ('CRI',2025,'2025a', 'Disponible INEC','Nuevo script',6,'Ronda mas reciente. Julio 2025. Condicion migratoria incluida'),
    ('PAN',2018,'2018a', 'Disponible ILO', 'Clonar+Run', 7,'Clonar 2019a. Verificar migracion -- posiblemente ausente'),
    ('PAN',2019,'2019a', 'Disponible ILO', 'Clonar+Run', 7,'Clonar 2020a'),
    ('PAN',2020,'2020a', 'Disponible ILO', 'Clonar+Run', 7,'EMLT (telefono). Documentar cambio metodologico'),
    ('PAN',2021,'2021a', 'Disponible ILO', 'Clonar+Run', 7,'Clonar 2022a'),
    ('PAN',2022,'2022a', 'Disponible ILO', 'Clonar+Run', 7,'Clonar 2023a. Referencia: abril'),
    ('PAN',2023,'2023a', 'Disponible ILO', 'Clonar+Run', 7,'Clonar 2024a'),
    ('PAN',2024,'2024a', 'Disponible ILO', 'Nuevo script',7,'Ronda mas reciente. Referencia: octubre 2024. Var migracion a verificar'),
]

# ═══════════════════════════════════════════════════════════════════════════════
# TABLA RESUMEN
# ═══════════════════════════════════════════════════════════════════════════════
# (pais, iso3, encuesta, freq, period_code, mas_reciente, n_rondas, mig_var, portal, notas)
SUMMARY = [
    # Grupo A
...
# Columna de letras para n dias variables (max 8)
_LETTERS = ['F','G','H','I','J','K','L','M','N']

def write_timeline_sheet(ws, tasks, title, subtitle, day_cols):
    """Render a Gantt timeline sheet. day_cols determines number of Gantt columns."""
    n_days   = len(day_cols)
    notes_col = _LETTERS[n_days]  # column after the last day
    last_col  = notes_col
    all_cols  = 'A:' + last_col

    # Column widths: A=#, B=Tarea, C=Pais, D=Ronda, E=Tipo, F..=days, last=Notas
    widths = [4, 52, 7, 9, 11] + [10] * n_days + [55]
    letters = ['A','B','C','D','E'] + _LETTERS[:n_days] + [notes_col]
    for col_l, w in zip(letters, widths):
        ws.column_dimensions[col_l].width = w

    total_cols = 5 + n_days + 1  # #,Tarea,Pais,Ronda,Tipo + days + Notas

    def merge_row(r, val, fnt, flll, alnn):
        ws.row_dimensions[r].height = 26 if r == 1 else 20
        ws.merge_cells(f'A{r}:{notes_col}{r}')
        c = ws[f'A{r}']
        c.value = val; c.font = fnt; c.fill = flll; c.alignment = alnn; c.border = bd()

    merge_row(1, title,
              Font(bold=True, size=12, color=WHITE, name='Calibri'),
              fill(HDR_BG), aln())
    merge_row(2, subtitle,
              Font(size=9, italic=True, color='555555', name='Calibri'),
              fill(LIGHT), aln(h='left'))

    ws.row_dimensions[3].height = 26
    hdrs = ['#', 'Tarea', 'Pais', 'Ronda', 'Tipo'] + day_cols + ['Notas']
    ...
    for task in tasks:
        ...
        if ttype == 'SEC':
            ws.merge_cells(f'A{row}:{notes_col}{row}')
            c = ws[f'A{row}']
            c.value = f'  {desc}'
        ...
        for di, col_l in enumerate(_LETTERS[:n_days], 1):
            c = ws[f'{col_l}{row}']
        ...
        wr(notes_col, notes, ha='left', it=True, clr='555555')
        row += 1
...
def write_inventory_sheet(ws, inventory, title, note):
    ...
    for ci, h in enumerate(['Pais','Ano','Ronda','Raw','Accion','Dia','Notas'], 1):
        c = ws.cell(row=3, column=ci)
        c.value = h
        c.font  = Font(bold=True, size=9, color=WHITE, name='Calibri')
        c.fill  = fill('555555'); c.alignment = aln(); c.border = bd()
...
# ── Hoja 1: Timeline Grupo A ──────────────────────────────────────────────────
ws1 = wb.active
ws1.title = 'Timeline A (COL PER ECU CHL)'
write_timeline_sheet(
    ws1, TASKS_A,
    'HDMF - Grupo A: COL(8) + PER(7) + ECU(8) + CHL(3) = 26 rondas | 6-10 Abril 2026',
...
# ── Hoja 2: Timeline Grupo B ──────────────────────────────────────────────────
ws2 = wb.create_sheet('Timeline B (6 nuevos paises)')
write_timeline_sheet(
    ws2, TASKS_B,
    'HDMF - Grupo B: DOM(8) + ARG(7) + PRY(8) + URY(7) + CRI(8) + PAN(7) = 45 rondas | 13-22 Abril 2026',
...
# ── Hoja 3: Inventario Grupo A ────────────────────────────────────────────────
ws3 = wb.create_sheet('Inventario - Grupo A')
write_inventory_sheet(
    ws3, INV_A,
    'Inventario rondas Grupo A | COL + PER + ECU + CHL | 2018-2025',
    'CHL CASEN bienal: solo existen 2020, 2022 y 2024 (2019 cancelada; 2018/2021/2023 no existen). '
    'COL GEIH: cambio de marco metodologico enero 2022 (2005->2018) -- dictionary check critico en ronda 2022 vs 2021.'
)

# ── Hoja 4: Inventario Grupo B ────────────────────────────────────────────────
ws4 = wb.create_sheet('Inventario - Grupo B')
write_inventory_sheet(
    ws4, INV_B,
    'Inventario rondas Grupo B | DOM + ARG + PRY + URY + CRI + PAN | 2018-mas reciente',
...
c = ws5['A1']
c.value = 'HDMF -- Resumen del Proyecto | 10 paises | 71 rondas | 2018-2025'
...
c = ws5['A2']
c.value = ('  Grupo A (existentes): COL 8 rondas + PER 7 + ECU 8 + CHL 3 = 26 rondas     |     '
           'Grupo B (nuevos): DOM 8 + ARG 7 + PRY 8 + URY 7 + CRI 8 + PAN 7 = 45 rondas     |     '
           'TOTAL: 71 rondas en 10 paises')
...
for ri, row_data in enumerate(SUMMARY[:4], 5):
    pais, iso3, survey, freq, period, recent, n_rondas, mig, portal, notes = row_data
    ws5.row_dimensions[ri].height = 28
    bg = LIGHT if ri % 2 == 0 else WHITE
    vals = [pais, iso3, survey, freq, period, recent, n_rondas, mig, portal, notes]
...
totals_data = [
    ...
    ('Total rondas',          '71',  'Grupo A: 26  |  Grupo B: 45'),
    ('Rondas ya hechas',      '5',   'COL 2025t3, COL 2024t3, ECU 2025m12, PER 2024a, CHL 2024a'),
    ('Rondas Grupo A nuevas', '21',  'Descargar + clonar/ejecutar en dias 1-5 (6-10 Abril)'),
    ('Rondas Grupo B nuevas', '45',  '6 scripts nuevos (survey_mapper) + 39 clones (13-22 Abril)'),
    ...
    ('Trabajo Grupo A',       '5 dias','Lun-Vie 6-10 Abril 2026. Descargas en paralelo dia 1'),
    ('Trabajo Grupo B',       '8 dias','Lun 13 - Mie 22 Abril 2026. Un pais por dia a partir del dia 2'),
]

for ri, (label, val, detail) in enumerate(totals_data, tot_row + 1):
    ws5.row_dimensions[ri].height = 18
    bg = LIGHT if ri % 2 == 0 else WHITE
    for ci, (txt, bld) in enumerate([(label,True),(val,True),(detail,False)], 1):
        c = ws5.cell(row=ri, column=ci)
        c.value = txt; c.font = Font(bold=bld, size=9, color=DARK, name='Calibri')
        c.fill  = fill(bg)
