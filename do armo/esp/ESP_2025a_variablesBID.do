...
clear
set more off
di "File created with the Claude HDMF system — 2026-04-29"

/*------------------------------------------------------------
  ESP_2025a_variablesBID.do
  Spain (ESP) — Encuesta de Población Activa (EPA), 2025 annual
  Input:  bases armo/raw/esp/ESP_2025a_merged.dta
  Output: bases armo/armo/ESP/ESP_2025a_BID.dta

  SCHEMA: EPA-TRIM-2021 (applies to 2021-2025 waves)
  To clone for another year: change local ANO only.
  For pre-2021 waves (2018-2020) see SCHEMA NOTE blocks.
...
* FACTOREL stored as integer × 10,000. Divide to recover actual weight.
...
******************
gen categopri_ci = .
replace categopri_ci = 1 if situ == 1 & emp_ci == 1
replace categopri_ci = 2 if situ == 3 & emp_ci == 1
replace categopri_ci = 3 if inlist(situ, 5, 7, 8) & emp_ci == 1
replace categopri_ci = 4 if situ == 6 & emp_ci == 1
...
/*===========================================================
  ...
  CNO-2011 1-digit maps 1-to-1 with ISCO-08 major groups:
    1=Managers, 2=Professionals, 3=Technicians, 4=Clerical,
    5=Service/Sales, 6=Agriculture, 7=Craft, 8=Machine operators,
    9=Elementary, 0=Armed forces (excluded)
  cno11: 1- or 4-digit CNO-2011 code (first digit = major group)
===========================================================*/
...
label var ocupa_ci "ISCO-08 major group via CNO-2011 (1-to-1 mapping)"

***********
* overqualified_ci
* edu_hdmf >= 7 in ESP 10-level scale = ISCED 5+ (technical/vocational and above)
* (equivalent to edu_hdmf >= 6 in the 8-level LAC scale used by COL/CHL/ECU/PER/USA)
...
/*===========================================================
  SCHEMA NOTE — Pre-2021 differences (2018-2020)
  When cloning this script for 2018-2020 waves, note:
  - Secondary-job variable block shifted position (build_epa_annual.py
    handles this in COLS_PRE2021; merged DTA has same column names).
  - ANORE1 (not ANORE) was the official variable name in design docs,
    but build script reads same bytes as 'anore' — no change needed here.
  - HORASH (not HORASH1) for usual hours in main job in 2018-2020.
    Variable is named 'horash' in pre-2021 merged DTAs.
    Replace `horash1` references with `horash` for those waves.
  No other changes required for 2021-2025.
===========================================================*/
