...
/*------------------------------------------------------------
  ESP_2020a_variablesBID.do
  Spain (ESP) — Encuesta de Población Activa (EPA), 2020 annual
  Input:  bases armo/raw/esp/ESP_2020a_merged.dta
  Output: bases armo/armo/ESP/ESP_2020a_BID.dta

  SCHEMA: EPA pre-2021 (Pet-2014, 152 vars).
  build_epa_annual.py normalises all column names to the same list
  as 2021+, EXCEPT: 'horash' (pre-2021) vs 'horash1' (2021+).
  The harmonisation code uses 'horase' (actual hours) and
  'horeplu' (both present in all years) — no change needed.
...
* Pre-2021 EPA: busca is asked of ALL not-employed; desea is only asked of busca==6
* respondents (non-searchers), so busca==1 & desea==1 always yields 0. Use busca==1 alone.
* Additionally, the raw positions for busca/desea in build_epa_annual.py were corrected
* (141-142 and 142-143) to fix a near-zero unemployment count in all pre-2021 waves.
...
***********
* horaspri_ci — usual hours worked in main job (HORASH, pre-2021 waves)
* QA fix 2026-05-04: horase = effective reference-week hours (inflates parcial_ci to ~30%+)
*   horash = usual/habitual hours → correct ~16% part-time, matches official ESP rate
*   Note: 2020 COVID year — ERTE furloughs increase gap between effective and usual hours
***********
gen double horaspri_ci = .
replace    horaspri_ci = horash if emp_ci == 1
