...
/***************************************************************************
             ...
             País: Perú | Encuesta: ENAHO | Año: 2018 | Ronda: Annual
             Cloned from PER_2024_variablesBID.do — differences from reference:
               [1] cotizando_ci: uses p524b1/p538b1 (p558b2 not available before 2024)
               [2] afiliado_ci: starts with 0 (not .), then overrides
               [3] relacion_ci=4: no p203==11 condition (added in 2019+)
             NOTE: no p518==99 fix needed for 2018 (only affects 2019-2022)
...
*************************************************************************** */
	...
	**relacion_ci**
	**************
	/* ALTERNATIVE [dictionary_check_PER 2018a]:
	   p203==11 (empleador doméstico familiar) was not included in 2018 ENAHO.
	   Category 4 maps only to p203 in [4,7]. Added from 2019 onwards.
	   Source: PER_2018a_variablesBID.do (MECOVI reference). */
	gen byte relacion_ci=.
	replace relacion_ci = 1 if p203 == 1
	replace relacion_ci = 2 if p203 == 2
	replace relacion_ci = 3 if p203 == 3
	replace relacion_ci = 4 if p203 >= 4 & p203 <= 7   // 2018: no p203==11
	...
	replace condocup_ci = 1 if p501==1 | p502==1 | p503==1
	replace condocup_ci = 2 if p501==2 & p502==2 & p503==2
	replace condocup_ci = 3 if condocup_ci == 2 & ( p5041==2 & p5042==2 & p5043==2 & p5044==2 & p5045==2 & p5046==2 & p5047==2 & p5048==2 & p5049==2 & p50410==2 & p50411==2 )
	...
	/* NOTE: no p518==99 cleaning for 2018 (non-response code only in 2019-2022) */
	...
	***************
	/* ALTERNATIVE [dictionary_check_PER 2018a]:
	   ...
	   Using actual contribution amounts: p524b1 (primary) and p538b1 (secondary) > 0.
	   Source: PER_2018a_variablesBID.do (MECOVI reference, same logic). */
