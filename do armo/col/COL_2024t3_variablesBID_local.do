...
di "File created with the Claude HDMF system — 2026-04-01"
...
use "C:\Users\PABLOCOR\OneDrive - Inter-American Development Bank Group\Archivos de Paraiso Pinto Furtado Luzes, Marta - Equipo Conocimiento\Datos\hdmf\How-do-Migrants-fare-in-LAC\bases armo\raw\col\COL_2024t3.dta", clear
...
// Secundaria 
replace aedu_ci = 5 + p3042s1 if inlist(p3042,4)
...
replace edu_hdmf = 6 if inlist(p3042,7,8,9) 
replace edu_hdmf = 5 if inlist(p3042,7,8,9) & inlist(p3043,1,2,3)

* 7. universitaria completa
replace edu_hdmf = 7 if inlist(p3042,10) 
replace edu_hdmf = 5 if inlist(p3042,10) & inlist(p3043,1,2,3)
replace edu_hdmf = 6 if inlist(p3042,10) & inlist(p3043,4,5,6)

* 8	. posgrado
replace edu_hdmf = 8 if  inlist(p3042,11,12,13) 
replace edu_hdmf = 7 if  inlist(p3042,11,12,13) & inlist(p3043,7)
