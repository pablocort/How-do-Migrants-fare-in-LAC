...
Referencia SCL: DOM_2020t4_variablesBID.do (alternative_do_files)
...
DIFERENCIAS RESPECTO A 2024:
  ...
  - aedu_ci: base secundaria = +8 (confirmar con SCL 2020)
  - destring: incluidos (2020 puede tener variables string — confirmar)
  - FX: 1 USD ≈ 52.78 DOP (PENDIENTE verificar tasa Q4 2020 en diccionario)
  - lp_ci/lpe_ci: valores de 2019 (PENDIENTE actualizar con MEPYD 2020)
  - salmm_ci: PENDIENTE verificar salario mínimo 2020
  AVISO: Este script fue generado sin leer DOM_2020t4 (archivo muy grande).
  Verificar con el archivo SCL de referencia antes de correr.
...
log using "`log_dir'/DOM_2020t4_variablesBID.log", replace
...
if c(username)=="PABLOCOR" {
	use "C:\Users\PABLOCOR\OneDrive - Inter-American Development Bank Group\Archivos de Paraiso Pinto Furtado Luzes, Marta - Equipo Conocimiento\Datos\hdmf\How-do-Migrants-fare-in-LAC\bases armo\raw\dom\DOM_2020t4.dta", clear
	local base_out "C:\Users\PABLOCOR\OneDrive - Inter-American Development Bank Group\Archivos de Paraiso Pinto Furtado Luzes, Marta - Equipo Conocimiento\Datos\hdmf\How-do-Migrants-fare-in-LAC\bases armo\armo\DOM\DOM_2020t4_BID.dta"
...
/* 2020: destring por precaución — verificar si la variable es string en el raw */
...
* Tasa de cambio Q4 2020: 1 USD ≈ 58.53 DOP (PENDIENTE confirmar), EUR factor = 0.9, CHF = 51.54
gen pension_int = pension_ext_monto                         if pension_ext_moneda=="DOP"
replace pension_int = pension_ext_monto * 58.53             if pension_ext_moneda=="USD"
replace pension_int = (pension_ext_monto * 0.9) * 58.53    if pension_ext_moneda=="EUR"
replace pension_int = pension_ext_monto * 51.54             if pension_ext_moneda=="CHF"
replace pension_int = . if pension_ext==2

/* Módulo de remesas — verificar estructura de variables en raw 2020 */
forvalues y = 1/6 {
	forvalues x = 1/3 {
		gen remesasaux`y'_`x' = mes`y'_`x'_ext_monto if (mes1_1_ext_moneda=="DOP" | mes2_1_ext_moneda=="DOP" | mes3_1_ext_moneda=="DOP")
		replace remesasaux`y'_`x' = mes`y'_`x'_ext_monto * 58.53       if (mes1_1_ext_moneda=="USD" | mes2_1_ext_moneda=="USD" | mes3_1_ext_moneda=="USD")
		replace remesasaux`y'_`x' = (mes`y'_`x'_ext_monto * 0.9)*58.53 if (mes1_1_ext_moneda=="EUR" | mes2_1_ext_moneda=="EUR" | mes3_1_ext_moneda=="EUR")
	}
}
...
/* 2020: base +8 para secundaria (confirmar con SCL 2020) */
...
label var aedu_ci "Anios de educacion aprobados (base +8 secundaria; pendiente verificar)"
...
/* PENDIENTE: actualizar lp/lpe con valores MEPYD 2020 */
gen lp_ci = .
replace lp_ci = 5319.5 if zona==1
replace lp_ci = 4736.2 if zona==2
label var lp_ci "Linea de pobreza oficial del pais (DOP mensuales, 2020 — PENDIENTE)"

gen lpe_ci = .
replace lpe_ci = 2395.2 if zona==1
replace lpe_ci = 2295.0 if zona==2
label var lpe_ci "Linea de pobreza extrema del pais (DOP mensuales, 2020 — PENDIENTE)"

/* PENDIENTE: verificar salario mínimo 2020 */
gen salmm_ci = 14582.5
label var salmm_ci "Salario minimo legal mensual (DOP, 2020 — PENDIENTE confirmar)"
