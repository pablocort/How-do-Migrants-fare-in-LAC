...
Referencia SCL: DOM_2021t4_variablesBID.do (alternative_do_files)
...
DIFERENCIAS RESPECTO A 2024:
  ...
  - aedu_ci: base secundaria = +8 (sistema anterior; cambia a +6 en 2022)
  - destring: incluidos (confirmado en SCL 2021)
  - FX: 1 USD = 52.78 DOP, EUR factor = 0.9, CHF = 51.54 DOP
  - lp_ci/lpe_ci: zona-específicas (mismos valores que 2019 per SCL —
    valores actualizados pueden diferir)
  - salmm_ci = 9411.6 (valor stale en SCL; verificar salario real 2021)
...
log using "`log_dir'/DOM_2021t4_variablesBID.log", replace
...
if c(username)=="PABLOCOR" {
	use "C:\Users\PABLOCOR\OneDrive - Inter-American Development Bank Group\Archivos de Paraiso Pinto Furtado Luzes, Marta - Equipo Conocimiento\Datos\hdmf\How-do-Migrants-fare-in-LAC\bases armo\raw\dom\DOM_2021t4.dta", clear
	local base_out "C:\Users\PABLOCOR\OneDrive - Inter-American Development Bank Group\Archivos de Paraiso Pinto Furtado Luzes, Marta - Equipo Conocimiento\Datos\hdmf\How-do-Migrants-fare-in-LAC\bases armo\armo\DOM\DOM_2021t4_BID.dta"
...
/* 2021: destring confirmado en SCL */
...
* Tasa de cambio Q4 2021: 1 USD = 52.78 DOP, EUR factor = 0.9, CHF = 51.54 DOP
...
/* 2021: base +8 secundaria (SCL usa +8; cambia a +6 en 2022) */
...
/* 2021: mismos valores que 2019 per SCL — verificar con MEPYD 2021 */
gen lp_ci = .
replace lp_ci = 5319.5 if zona==1
replace lp_ci = 4736.2 if zona==2
label var lp_ci "Linea de pobreza oficial del pais (DOP mensuales, 2021)"

gen lpe_ci = .
replace lpe_ci = 2395.2 if zona==1
replace lpe_ci = 2295.0 if zona==2
label var lpe_ci "Linea de pobreza extrema del pais (DOP mensuales, 2021)"

/* 2021: SCL tiene 9411.6 (valor stale — salario mínimo real 2021 es mayor) */
gen salmm_ci = 9411.6
label var salmm_ci "Salario minimo legal mensual (DOP, 2021 — verificar; SCL usa valor de 2017)"
