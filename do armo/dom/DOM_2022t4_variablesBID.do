...
Referencia SCL: DOM_2022t4_variablesBID.do (alternative_do_files)
...
DIFERENCIAS RESPECTO A 2024:
  ...
  - aedu_ci: base secundaria = +6 (igual que 2022-2024; BREAK con 2018-2021 que usan +8)
  - destring: incluidos (confirmado en SCL 2022)
  - tipocontrato_ci: condición extra contrato_verbal_escrito==2 (presente en SCL 2022)
  - FX: 1 USD = 52.78 DOP (stale; SCL 2022 mantiene tasa de 2019)
  - lp_ci/lpe_ci: zona-específicas (urbana/rural), mismos valores que 2021 per SCL
  - salmm_ci = 16262.5 (confirmado en SCL 2022)
...
log using "`log_dir'/DOM_2022t4_variablesBID.log", replace
...
if c(username)=="PABLOCOR" {
	use "C:\Users\PABLOCOR\OneDrive - Inter-American Development Bank Group\Archivos de Paraiso Pinto Furtado Luzes, Marta - Equipo Conocimiento\Datos\hdmf\How-do-Migrants-fare-in-LAC\bases armo\raw\dom\DOM_2022t4.dta", clear
	local base_out "C:\Users\PABLOCOR\OneDrive - Inter-American Development Bank Group\Archivos de Paraiso Pinto Furtado Luzes, Marta - Equipo Conocimiento\Datos\hdmf\How-do-Migrants-fare-in-LAC\bases armo\armo\DOM\DOM_2022t4_BID.dta"
...
/* 2022: ID numérico via group() — concat() empieza en 2023 */
sort vivienda hogar
...
/* 2022: destring confirmado en SCL */
...
/* 2022: condición adicional contrato_verbal_escrito==2 confirmada en SCL 2022 */
gen tipocontrato_ci = .
...
* Tasa de cambio Q4 2022: SCL mantiene 1 USD = 52.78 DOP (stale — tasa real 2022 ≈ 55 DOP)
...
Sistema educativo de República Dominicana (ENCFT):
  nivel_ultimo_ano_aprobado:
    ...
    5  = Universitario     → aedu=último+12
    6  = Post-grado        → aedu=último+16
    7  = Maestría          → aedu=último+16
    8  = Doctorado         → aedu=último+18
    9  = Ninguno           → aedu=0, HDMF=1
   10  = Quisqueya Aprende → aedu=0, HDMF=1
   99  = Otro/NS           → missing

NOTA 2022: base secundaria = +6 (sistema actual de 6 años de primaria).
  Años anteriores (2018-2021) usaban +8 (sistema antiguo de 8 años).
*/
...
/* 2022: base +6 (SCL confirma; corrección del sistema antiguo de +8) */
...
* Nota: la ENCFT no recoge tiempo de residencia; variable no disponible en ninguna onda.
...
/* 2022: mismos valores zona-específicos que 2021 per SCL — verificar con MEPYD 2022 */
gen lp_ci = .
replace lp_ci = 5319.5 if zona==1
replace lp_ci = 4736.2 if zona==2
label var lp_ci "Linea de pobreza oficial del pais (DOP mensuales, 2022)"

gen lpe_ci = .
replace lpe_ci = 2395.2 if zona==1
replace lpe_ci = 2295.0 if zona==2
label var lpe_ci "Linea de pobreza extrema del pais (DOP mensuales, 2022)"

/* 2022: salmm_ci = 16262.5 (confirmado en SCL 2022) */
gen salmm_ci = 16262.5
label var salmm_ci "Salario minimo legal mensual (DOP, 2022)"
