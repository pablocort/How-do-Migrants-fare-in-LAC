*==============================================================================
* HND_2023m6_variablesBID.do
* Honduras EPHPM — June 2023
* HDMF harmonization script
* NEW STRUCTURE: condact; id+orden for IDs; day-by-day hours;
*   FULL non-labor income (tc_c1=24.7285, /3); salmm=12377.73;
*   razonesnoasis_ci; repiteult_ci from ed11; diversity from ch308
*==============================================================================
clear
set more off
...
log using "`log_dir'/HND_2023m6_variablesBID.log", replace

use "bases armo/raw/hnd/HND_2023m6.dta", clear

* No rename loop — variables already lowercase
...
*------------------------------------------------------------------------------
gen region_c = .
label var region_c "Department (not available 2023)"
...
*------------------------------------------------------------------------------
* Household and person identifiers — id and orden
*------------------------------------------------------------------------------
gen str idh_ch = ""
capture {
    tostring id, replace
...
label var idh_ch "Household ID (id)"

gen str idp_ci = ""
capture {
    tostring orden, replace
    gen idp_ci = idh_ch + "_" + orden
}
label var idp_ci "Person ID (id+orden)"
...
*------------------------------------------------------------------------------
* Migration — NOT available in 2023
*------------------------------------------------------------------------------
gen migrante_ci     = .
gen mig_pais_ci     = .
gen migrantiguo5_ci = .
...
*------------------------------------------------------------------------------
* Diversity — ch308: ethnicity/race
*------------------------------------------------------------------------------
gen afro_ci = .
gen ind_ci  = .
capture {
    replace afro_ci = 1 if inlist(ch308, 4, 5)    // afro-honduran, garifuna
    replace afro_ci = 0 if !inlist(ch308, 4, 5) & !missing(ch308)
    replace ind_ci  = 1 if inrange(ch308, 1, 3)   // indigenous groups
...
*------------------------------------------------------------------------------
* Employment — condact (lowercase in 2023)
*------------------------------------------------------------------------------
gen condocup_ci = .
...
* 2023 has social security module — cotizando from IHSS contribution
*------------------------------------------------------------------------------
gen cotizando_ci = .
gen afiliado_ci  = .
capture {
    gen cotizando_ci = 0 if emp_ci == 1
    replace cotizando_ci = 1 if emp_ci == 1 & ///
        (inrange(oc617_1,1,5) | inrange(oc617_2,1,5) | inrange(oc617_3,1,5))
}
gen formal_ci = cotizando_ci if emp_ci == 1
label var cotizando_ci "Contributing to SS=1"
...
*------------------------------------------------------------------------------
* Education — ed05/ed08/ed10/ed13 (same as 2022)
*------------------------------------------------------------------------------
gen aedu_ci = .
capture {
...
*------------------------------------------------------------------------------
* Income — primary job (quarterly → /3)
*------------------------------------------------------------------------------
local tc = `tc_c1'
...
*------------------------------------------------------------------------------
* Non-labor income — FULL calculation (unlike 2022)
* oih variables quarterly → /3; USD → HNL via tc_c1
*------------------------------------------------------------------------------
gen ynlm_ci = 0
...
gen tc_c1 = `tc_c1'
label var tc_c1 "Exchange rate HNL/USD (2023: 24.7285)"
...
saveold "bases armo/armo/HND/HND_2023m6_BID.dta", replace
di "HND_2023m6_BID.dta saved. N = " _N
