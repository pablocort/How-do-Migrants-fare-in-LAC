*==============================================================================
* HND_2024m6_variablesBID.do
* Honduras EPHPM — June 2024
* HDMF harmonization script
* NEW STRUCTURE: CONDACT uppercase; depmuestra for region_c;
*   tothrsop/thoras for hours; ynlm_ci=.; remesas_ci=.;
*   tc_c1=25.12562814; poverty from pobreza/yperhg; dis_ci from ch307
*==============================================================================
clear
set more off
...
log using "`log_dir'/HND_2024m6_variablesBID.log", replace

use "bases armo/raw/hnd/HND_2024m6.dta", clear

* No rename loop — variables already in expected case
...
local tc_c1 = 25.12562814
* TODO: verify exact 2024 minimum wage; using INE-published June 2024 value
local salmm = 13601.28
...
* hogar is already a string in 2024; orden is int8
*------------------------------------------------------------------------------
clonevar idh_ch = hogar
label var idh_ch "Household ID"

tostring orden, gen(orden_str)
gen idp_ci = strtrim(hogar) + "_" + orden_str
label var idp_ci "Person ID (hogar+orden)"
drop orden_str
...
*------------------------------------------------------------------------------
* Diversity — ch307 disability; ch308 ethnicity
*------------------------------------------------------------------------------
gen dis_ci = .
capture {
...
*------------------------------------------------------------------------------
* Migration — NOT available in 2024
*------------------------------------------------------------------------------
gen migrante_ci     = .
gen mig_pais_ci     = .
gen migrantiguo5_ci = .
...
*------------------------------------------------------------------------------
* Formality — not available in 2024
*------------------------------------------------------------------------------
gen cotizando_ci = .
gen afiliado_ci  = .
gen formal_ci    = .
label var cotizando_ci "Social security (not available 2024)"
label var formal_ci    "Formal (not available 2024)"

gen salmm_ci = `salmm'
label var salmm_ci "Monthly minimum wage (HNL, TODO: verify)"
...
* Non-labor income and remittances — NOT calculated (follow 2022 reference)
gen ynlm_ci    = .
gen remesas_ci = .
gen remesas_ch = .
label var ynlm_ci    "Non-labor income (not calculated 2024)"
label var remesas_ci "Remittances (not calculated 2024)"
...
*------------------------------------------------------------------------------
* Poverty — from INE survey variables
*------------------------------------------------------------------------------
* lpe_ci: extreme poverty line (June 2024 INE values)
gen lpe_ci = .
replace lpe_ci = 2457.16 if zona_c == 1    // urban
replace lpe_ci = 1895.23 if zona_c == 0    // rural
label var lpe_ci "Extreme poverty line (HNL per capita/month)"

gen ln_ci = .
replace ln_ci = 5131.84 if zona_c == 1     // urban
replace ln_ci = 2604.48 if zona_c == 0     // rural
...
*------------------------------------------------------------------------------
gen tc_c1 = `tc_c1'
label var tc_c1 "Exchange rate HNL/USD (June 2024)"
